...
const baseCustomerFields = {
  ...
  location: z.string().max(200, "Location is too long").optional().default(""),
  orders: z.coerce.number().int().min(0, "Orders count cannot be negative").optional().default(0),
  spent: z.string().max(30).optional().default("0"),
  segment: z.enum(["all", "new", "europe", "returning"]).optional().default("new"),
  email: z.string().email("Email must be valid").max(254, "Email is too long").optional(),
};
